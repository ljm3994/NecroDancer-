...
Image::Image(_tstring file, string ps):parent(nullptr)
{
	...
	assert(SUCCEEDED(hr)&& "리소스자원뷰생성실패");
...
void Image::Render(ID3D11DeviceContext * dc)
{
	Update();
	UINT stride = sizeof(Vertex);
	UINT offset = 0;
	...
	//색상 값
	dc->UpdateSubresource(colorBuffer, 0, NULL, &colorBufferData, 0, 0);
	dc->PSSetConstantBuffers(0, 1, &colorBuffer);
	...
	if (maxFrameX != 1 || maxFrameY != 1)
	{
		...
		FrameUvBufferData.FrameUv.z = 1.0f / maxFrameX * (CurrentFrameX + 1);
		FrameUvBufferData.FrameUv.w = 1.0f / maxFrameY * (CurrentFrameY + 1);
		dc->UpdateSubresource(FrameUvBuffer, 0, NULL, &FrameUvBufferData, 0, 0);
		dc->VSSetConstantBuffers(2, 1, &FrameUvBuffer);
	}

	shader->SetBuffers(dc);
	//픽섹 셰이더에 셰이더 자원뷰값 셋팅
	dc->PSSetShaderResources(0,//스타트 슬롯
		1,//리소스 갯수
		&srv);//셰이더자원뷰
	dc->IASetVertexBuffers(0, 1, &vertexBuffer, &stride, &offset);
	dc->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	//블렌드 켜기
	dc->OMSetBlendState(blendState[1], NULL, 0xff);
	dc->Draw(VertexCount, 0);
	//블렌드 끄기
	dc->OMSetBlendState(blendState[0], NULL, 0xff);
}
