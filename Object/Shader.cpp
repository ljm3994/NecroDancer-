...
void Shader::SetBuffers(ID3D11DeviceContext * dc)
{
	assert(vertexLayout != NULL);

	dc->UpdateSubresource(worldBuffer, 0, NULL, &worldBufferData, 0, 0);
	dc->VSSetConstantBuffers(1, 1, &worldBuffer);

	dc->VSSetShader(vertexShader, 0, 0);
	dc->PSSetShader(pixelShader, 0, 0);

	dc->IASetInputLayout(vertexLayout);
}
