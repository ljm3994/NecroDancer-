...
bool bThread = false;
bool bLoding = false;
ID3D11Multithread * mutilethread;
MainGame*	Mg;
                      ...
                      int       nCmdShow)
{
	...
	//Create Device and DeviceContext, SwapChain
	{
		...
		(
			...
			&Device,	//생성된 디바이스 인터페이스의 포인터를 담을 변수를 설정합니다.
			NULL, //생성에 성공한 경우에는 pFeatureLevels에서 지정했던 배열의 처음값을 돌려줍니다.실패한 경우에는 0이 반환됩니다.
			&DeviceContext	//생성된 디바이스 컨텍스트 인터페이스의 포인터를 담을 변수를 설정합니다.
		);

		ID3D11DeviceContext* deDc = nullptr;

		Device->CreateDeferredContext(0, &deDc);
	...
	Device->QueryInterface(IID_PPV_ARGS(&mutilethread));

	mutilethread->SetMultithreadProtected(true);

	mutilethread->Release();
	...
	//Create Viewport
	{
		...
		DeviceContext->RSSetViewports(1,//바인딩 할 뷰포트 수입니다.
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case MCIWNDM_NOTIFYPOS:
		if (VIDEOMANAGER->GetVideoHandel() == (HWND)wParam && VIDEOMANAGER->GetEnd() == lParam)
		{
			VIDEOMANAGER->Stop();
		}
	}
